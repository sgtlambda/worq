...
MagicHandle.prototype._runThread = function (thread) {
    var job = thread.job;
    job.emit(MagicHandle.events.START);
...
MagicHandle.prototype._processQueue = function () {
    if (this.hasJobsPending()) {
        ...
        this.emit(MagicHandle.events.IDLE);
    }
};
...
MagicHandle.prototype._considerClose = function () {
    if (this.isClosed() || this._state === MagicHandle.states.CLOSING)
        return Q();
...
MagicHandle.prototype.process = function (data) {
    ...
    this._processQueue();
    job.once(MagicHandle.events.START, function () {
        deferred.notify(MagicHandle.events.START);
    });
 ...
 * @returns {Promise}
 */
MagicHandle.prototype.close = function (graceful) {
    ...
    if (this.hasActiveThreads() || this.hasJobsPending()) {
        var defered = Q.defer();
        this.once(MagicHandle.events.IDLE, function () {
